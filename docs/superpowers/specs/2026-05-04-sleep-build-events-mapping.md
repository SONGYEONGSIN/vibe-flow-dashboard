# Brainstorm: sleep_build_* 이벤트 캐릭터 매핑

## 의도

- **산출물**: dashboard `event-map.ts` + `dialogue-pool.json`에 `sleep_build_start` / `sleep_build_done` / `sleep_build_abort` 3 이벤트 매핑 추가. 각 이벤트 → 1+ 캐릭터(developer/qa/planner) 액션 + 대사 풀.
- **사용자**: vibe-flow maker. 야간 자율 사이클 종료 후 dashboard `/characters` 에서 캐릭터 시각 피드백을 받는 시점. 대체 행동: jsonl 로그 직접 grep.
- **트리거**: vibe-flow PR #30(sleep-build Phase 1) 머지 직전 dogfooding. 매핑 부재 시 사이클 종료가 dashboard에 닿아도 12 캐릭터 무반응 → 정서적 피드백 루프 절단. 미루면 sleep-build 머지 후 maker 첫 사이클이 무반응으로 끝남.
- **성공**: events.jsonl에 `sleep_build_done` 1줄 → developer 캐릭터 액션 + 대사 1개. 단위 테스트 1+ 케이스 통과.
...
- **agent-event-map.ts (Stage 자동 진화)**: sleep_build_* 이벤트도 Stage 카운트에 가중 — developer/qa/planner에 매핑.

## 대안 비교

| 항목 | A. 3 이벤트 모두 매핑 | B. done만 매핑 (단순화) | C. 모든 12 캐릭터에 broadcast | Z. 매핑 없이 머지 |
|------|-------------------|----------------------|-------------------------|----------------|
| 핵심 | start/done/abort 3 매핑, 각 1~2 캐릭터 액션 | done만 매핑, start/abort는 무시 | 매 이벤트마다 12 캐릭터 모두 반응 | sleep-build PR 그대로 머지 |
...
**기각 Z**: maker 첫 사이클 무반응 — sleep-build 머지의 정서적 가치 절반 손실.

## 다음 단계

- spec 저장: `.claude/memory/brainstorms/20260504-105944-sleep-build-events-mapping.md`
- 구현 단위 (예상 4~5 파일):
  - `src/app/characters/data/event-map.ts` — `mapEvent` 분기 3개 추가
  - `src/app/characters/data/dialogue-pool.json` — developer/qa/planner 각각 sleep_build_* 컨텍스트 키 + 대사 풀
  - `src/app/characters/data/agent-event-map.ts` — Stage 카운트 룰에 `sleep_build_*` 추가
